...
Тарифы, подписки и платежи через общий порт `PaymentProvider` (адаптеры Payme и
Click). Валюты/планы — глобальные справочники; подписки/платежи — тенантные, без
DELETE (финансовая история). Платежи идемпотентны, вебхуки переживают повторную
доставку.
...
- **Порт `PaymentProvider`** (`ports.py`): `create_checkout`, `parse_webhook`,
  ...
  включаются `ENABLED_PAYMENT_PROVIDERS`.
