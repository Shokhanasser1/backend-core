...
class FileService(Service):
    ...
    async def create_thumbnail(
        self, source_file_id: UUID, *, max_edge: int | None = None
    ) -> FileDTO:
        """Generate a resized, metadata-stripped variant of an owned image and store
        it as a NEW file (its own row + storage key). The source is read through the
        backend; the generated bytes go back through ``upload`` — so a thumbnail is
        validated by the same magic-bytes allowlist (defence in depth). 404 for a
        foreign/missing source; a source that cannot be decoded is a 422."""
        source = await self._repo.get_or_raise(source_file_id)
        original = await self._storage.get(source.storage_key)
        edge = max_edge if max_edge is not None else self._settings.files_thumbnail_max_edge
        thumb_bytes, content_type = await self._thumbnailer.generate(original, max_edge=edge)
        return await self.upload(
            filename=None, declared_content_type=content_type, data=thumb_bytes
        )
