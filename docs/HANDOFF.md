...
## Обновление 2026-07-08 (2) — превью/тумбнейлы к product_images (принято, тег `v0.7.1`, закоммичено)

Продолжение v1.1 по отдельной команде владельца. **Принято и закоммичено, тег
`v0.7.1`** (в origin не пушено — по решению владельца):
...
- **Статус:** закоммичено, тег `v0.7.1`. Не запушено в origin (по решению
  владельца) — при желании: `git push origin main && git push origin v0.7.1`.
