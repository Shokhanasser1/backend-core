...
Чек-лист активации тенанта. Решение Фаза-0-стиля **утверждено владельцем** (та же
wildcard-развилка что у metering): **шаги отмечаются ЯВНО** —
`OnboardingService.complete_step(step)` (серверный glue/фичи на вехе) +
аутентифицированный `POST /api/saas/onboarding/steps/{step}/complete` (фронт);
НЕ подписки на шину.

- **`saas.onboarding`** (`modules/saas/onboarding/`, requires core auth/tenants) —
  прогресс по настраиваемым шагам.
  - **Таблица** `saas_onboarding_progress` (тенантная RLS, ветка `saas_onboarding`):
    строка на выполненный шаг (`(tenant, step_key)` уникальны), отсутствие = не
    выполнен. **Набор шагов — конфигом** `SAAS_ONBOARDING_STEPS` (клиент задаёт).
  - **`OnboardingService`:** `complete_step` (идемпотентно; ключ вне набора → 422;
    публикует `saas.onboarding.completed` ровно раз — на последнем шаге),
    `progress()` (`completed_count`/`total`/`is_complete`). Права
    `saas.onboarding:read|update` (owner/admin), роуты `GET /me` + `POST …/complete`.
