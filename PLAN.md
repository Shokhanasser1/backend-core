...
- 2026-07-08 — **Модуль `saas`, этап 3 — фича `saas.onboarding` ПРИНЯТА,
  ...
  Фаза-0-стиля (утв. владельцем, та же wildcard-развилка что у metering): **шаги
  отмечаются ЯВНО** — `OnboardingService.complete_step(step)` (серверный glue/фичи
  на вехе) + аутентифицированный `POST /api/saas/onboarding/steps/{step}/complete`
  (фронт отмечает пользовательские шаги); НЕ подписки на шину (фичам запрещён
  wildcard). Фича `saas.onboarding` (requires core auth/tenants): таблица
  `saas_onboarding_progress` (тенантная RLS, ветка `saas_onboarding`) — строка на
  выполненный шаг (`(tenant, step_key)` уникальны), отсутствие = не выполнен.
  **Набор шагов — конфигом** `SAAS_ONBOARDING_STEPS` (клиент задаёт свой путь;
  шаблон не зашивает; complete_step валидирует ключ ∈ набор → 422). `OnboardingService`:
  `complete_step` (идемпотентно; публикует `saas.onboarding.completed` ровно раз —
  когда закрыт последний шаг), `progress()` (по шагу на каждый сконфигурированный +
  `completed_count`/`total`/`is_complete`). Права `saas.onboarding:read|update`
  (owner/admin), роуты `GET /me` + `POST …/complete`. Событий не слушает.
  **Ретенции нет** (прогресс — постоянное состояние активации, не временной ряд).
  Проверки: **348 тестов зелёные (+6), покрытие 94.35%** (вся фича 100%, включая
  сервис), ruff/mypy strict (172 файла)/import-linter чисто, миграции всех веток
  обратимы (+`saas_onboarding`). Тесты целиком через HTTP (`saas_onboarding_client`
  с настроенными шагами): прогресс, POST-завершение, идемпотентность, 422 на чужой
  шаг, изоляция тенантов, 403/401. Приёмка владельцем пройдена (тесты зелёные) →
  модуль saas (начальный объём) завершён; далее — по команде владельца
  (crm / tg-bot / реальный клиент / доработки saas).
